<?php
  ...
  $sql1 = "SELECT DISTINCT(".$config_uidField.") FROM `".$config_databaseTablePrefix."products_import`";
  ...
  if ($result1 = mysqli_use_result($link1))
  {
    ...
    while($product = mysqli_fetch_assoc($result1))
    {
      ...
      $sql2 = "SELECT SQL_CALC_FOUND_ROWS name,CASE merchant
        ...
        ELSE '999' END AS priority, brand FROM `".$config_databaseTablePrefix."products_import` WHERE ".$config_uidField."='".database_safe($product[$config_uidField])."' ORDER BY priority LIMIT 1";
    ...
    while($product = mysqli_fetch_assoc($result1))
    {
      ...
      $searchName = tapestry_search($normalisedName);
		...
		if ($normalisedBrand != '') { //Als Brand niet leeg is, neem dan de brand name over
			...
			$sql2 = "UPDATE `".$config_databaseTablePrefix."products_import` SET name='".database_safe($product["name"])."',normalised_name='".database_safe($normalisedName)."',search_name='".database_safe($searchName)."',brand='".database_safe($normalisedBrand)."' WHERE ".$config_uidField."='".database_safe($product["uid"])."'";
			
	} else {
		
		  $sql2 = "UPDATE `".$config_databaseTablePrefix."products_import` SET name='".database_safe($product["name"])."',normalised_name='".database_safe($normalisedName)."',search_name='".database_safe($searchName)."' WHERE ".$config_uidField."='".database_safe($product["uid"])."'";
